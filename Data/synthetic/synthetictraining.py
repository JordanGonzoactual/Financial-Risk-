...
def generate_time_based_features(num_samples):
    ...
    dates = [(start_date + timedelta(days=int(day))).strftime('%Y-%m-%d') for day in random_days]
    return dates
...
def loan_approval_rule(row):
    ...
    month = int(row['ApplicationDate'].split('-')[1])  # Extract month from YYYY-MM-DD string
...
print(f"\nSynthetic training data saved to '{output_path}'")
