...
def run_learning_rate_tuning(X_train, X_val, y_train, y_val, base_params, n_trials, parent_run_id, random_state=42):
    ...
    def objective(trial: optuna.trial.Trial) -> float:
        with mlflow.start_run(run_name=f"trial_{trial.number}", nested=True) as run:
            ...
            mlflow.set_tag("optimization_step", "learning_rate")
    ...
    logging.info(f"Best trial for learning rate tuning: {study.best_trial.value}")
    logging.info(f"Best parameters for learning rate tuning: {study.best_params}")
